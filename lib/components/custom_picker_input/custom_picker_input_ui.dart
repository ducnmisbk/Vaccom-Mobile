...
import 'custom_picker_input_controller.dart';
...
class _DropDownContentState extends State<DropDownContent> {
  ...
  Widget build(BuildContext context) {
    ...
    double searchHeight = showSearch ? 60 : 0;
    double padding = 16;
    double itemHeight = widget.type == DropDownType.multiSelect ? 90 : 56;
    double maxContainHeight = MediaQuery.of(widget.mainContext).size.height -
        MediaQuery.of(widget.mainContext).padding.top -
        MediaQuery.of(widget.mainContext).padding.bottom;
    int numberOfItem = widget.data != null ? dataFilter.length : 0;
    double containHeight =
        headerHeight + searchHeight + padding + itemHeight * numberOfItem;
    double height =
        containHeight < maxContainHeight ? containHeight : maxContainHeight;
